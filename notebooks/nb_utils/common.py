...
from notebooks.nb_utils.inspect_reproduced_results import display_exps_results_against_configs
...
FSVI_EXPS = os.path.join(root, "sfsvi/exps/ablation/runs/reproduce_main_results_2_simplified")
FSVI_EXPS_ORIG = os.path.join(root, "sfsvi/exps/ablation/runs/reproduce_main_results_2")
# folder of experiments for reproducing baselines results (e.g. FRCL, FROMP, VCL)
BASELINES_EXPS = os.path.join(root, "sfsvi/exps/ablation/runs/reproduce_main_results_3")
...
def display_results_of_cached_exp():
	...
	exps = lutils.read_folder(BASELINES_EXPS, only_config=True, only_successful=True)
...
def show_final_average_accuracy(training_log_df):
