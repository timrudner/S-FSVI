...
class Objectives_hk:
    ...
    def _elbo_fsviassification_multihead(
        ...
        @param inducing_inputs: an array with inducing points from all task, or a dictionary that maps
            task id to the inducing points
        ...
        for t_id in sorted(prior_means.keys()):
            ...
            inducing_inputs_task = inducing_inputs[t_id]
        ...
        n_inducing_inputs = sum([array.shape[0] for array in inducing_inputs.values()])
        scale = compute_scale(self.kl_scale, inputs, n_inducing_inputs, self.n_marginals)
        if loss_type == 1:
...
def _check_input(
    ...
    def _kvs(d: Dict):
        keys = sorted(d.keys())
        num_inducing_samples = [d[k].shape[0] for k in keys]
        assert keys[-1] <= task_id
        return keys, num_inducing_samples

    assert _kvs(prior_means) == _kvs(prior_covs) == _kvs(inducing_inputs)
