...
class CLPrior:
    ...
    def make_prior_fn(
        ...
        @return:
            prior_fn: a function that takes in a dict that maps from task id to inducing input points,
        ...
        if prior_type == "bnn_induced":
            ...
            # prior_fn is a function of inducing_inputs
            prior_fn = partial(
            ...
            def prior_fn(inducing_inputs: Dict):
                # it is always the latest task that needs random inducing points
                # TODO make this logic easier to understand
                max_key = max(list(inducing_inputs.keys()))
                x_len = len(inducing_inputs[max_key])
