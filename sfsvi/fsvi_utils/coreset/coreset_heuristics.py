...
def add_by_entropy(
    ...
    then use a certain heuristic to select inducing points from the samples.
    ...
    @param x_candidate: a batch of input samples to select inducing points from
...
def add_by_kl(
    ...
    dummy_task_id = 0
    prior_means, prior_covs = prior_fn_only_x(inducing_inputs={dummy_task_id: x_candidate})
