...
def _create_weights_mf(
    in_dim, hidden_size, out_dim, init_weights=None, init_variances=None
):
...
class Cla_NN(object):
    ...
    def assign_optimizer(self, learning_rate=0.001):
        self.train_step = tf.compat.v1.train.AdamOptimizer(learning_rate).minimize(
            self.cost
        )
    ...
    def prediction(self, x_test, task_idx):
        ...
        prediction = self.sess.run(
            [self.pred], feed_dict={self.x: x_test, self.task_idx: task_idx}
        )[0]
        return prediction

    def prediction_prob(self, x_test, task_idx):
        prob = self.sess.run(
            [tf.nn.softmax(self.pred)],
...
class Vanilla_NN(Cla_NN):
    ...
    def _prediction(self, inputs, task_idx):
        ...
        for i in range(self.no_layers - 1):
            ...
            act = tf.nn.relu(pre)
        pre = tf.add(
            tf.matmul(act, tf.gather(self.W_last, task_idx)),
            tf.gather(self.b_last, task_idx),
        )
        return pre
...
class MFVI_NN(Cla_NN):
    def __init__(
        ...
        self.prior_W_m, self.prior_b_m, self.prior_W_last_m, self.prior_b_last_m = (
            m[0],
            m[1],
            m[2],
            m[3],
        )
        self.prior_W_v, self.prior_b_v, self.prior_W_last_v, self.prior_b_last_v = (
            v[0],
            v[1],
            v[2],
            v[3],
        ...
        self.cost = tf.compat.v1.div(self._KL_term(), training_size) - self._logpred(
            self.x, self.y, self.task_idx
        )
    ...
    def _prediction_layer(self, inputs, task_idx, no_samples):
        ...
        for i in range(self.no_layers - 1):
            din = self.size[i]
            dout = self.size[i + 1]
            eps_w = tf.random.truncated_normal((K, din, dout), 0, 1, dtype=tf.float32)
            eps_b = tf.random.truncated_normal((K, 1, dout), 0, 1, dtype=tf.float32)
    ...
    def create_weights(
        self, in_dim, hidden_size, out_dim, prev_weights, prev_variances
    ):
        hidden_size = deepcopy(hidden_size)
        hidden_size.append(out_dim)
        hidden_size.insert(0, in_dim)
        ...
        return (
            [W_m, b_m, W_last_m, b_last_m],
            [W_v, b_v, W_last_v, b_last_v],
            hidden_size,
        )
